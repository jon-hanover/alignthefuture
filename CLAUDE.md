...
Venn in the watercolor logo colors above a table with the categories down the
left, stacking into tagged rows on phones. Its CSS lives in `assets/venn/`, so
the same markup can be dropped into any page. An animated version was tried and
dropped; it is in git history if wanted again.
